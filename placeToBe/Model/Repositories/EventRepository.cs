...
namespace placeToBe.Model.Repositories
{
    public class EventRepository: MongoDbRepository<Event>
    {
        ...
        public async Task<IList<Event>> GetCityMap(string place, string time)
        {
            var collection = GetCollection(); 
             /*    var query = Builders<Event>.Filter.GeoWithinPolygon("Location", test1.locationCoordinates.coordinates);
